...
## Owner answer, 2026-09-26: (b), bring the bank in to Hathaway's taper

*"ok yes st till want to bring that bulge in some, so the sidewalk is fairly straight and following, at least 50% of what is north of south water street i think"*, then, on seeing PR #88's per-block table: *"i think it is closer to the hathaway map in that reading"*.

This is option (b) as asked, and it replaces the "stay on Wright's ink" acceptance above for this reach only. The owner's second message sets the amount: **Hathaway's figures, not a halving of Wright's.** Halving Wright's 39 m at block 19 would have cut to ~19.5 m, past Hathaway, which is the overcorrection he warned against.

- **Target depth per block**, measured the way PR #88 measures (bank to the block tier's north line, inside each sheet): block 20 **≈29.3 m** (Wright 34.9), block 19 **≈28.2 m** (Wright 39.0), block 18 **≈24.2 m** (Wright 31.7), block 17 unchanged (~20 m). The bank comes in about 6 / 11 / 7.5 m, peaking at block 19, and tapers evenly from the bend near the forks to the La Salle mouth, keeping Wright's re-entrant as the one break. That removes the swell's maximum, so the bank narrows steadily as Hathaway draws it, and the walk can run fairly straight. State the achieved depth per block against these targets. Tolerance is the trace's own ink-distance p90 (~4 m).
...
- **Provenance.** The bank on this reach follows **Hathaway 1834** (`hathaway_1834`), graded `inferred` if placed off Hathaway under a stated registration, otherwise `reconstructed`, and recorded in docs/LIBERTIES.md either way. The record cites PR #88's reading (`data/traces/south_bank_swell_1834.json`) and this ruling. Wright's ink stays in the record as the set-aside reading. Update the `read_south_bank_swell_1834.py` gate so it records the deliberate departure instead of failing on it. It must not be deleted.
- **Acceptance:** blocks 20/19/18 stand at Hathaway's depths within ~4 m, stated per block. The walk reads straight from the bend to the slough crossing. No land or planting stands north of the new bank. `check.sh` and the named smoke legs are green at both widths.
