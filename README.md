# chicago-tickets — the 4D Chicago ticket queue

This repository is the **single operational answer to "what should be worked on next,
and what is everything in flight"** for [4D Chicago](https://chicago.polecat.live/4d/)
(code: [kevinrhaas/chicago](https://github.com/kevinrhaas/chicago)). Manager's
**4D Board** reads it; the owner orders `QUEUE.md`.

## Why this is its own repository (2026-09-23)

Until 2026-09-23 the tickets lived inside the code repo at `chicago/4d/tickets/`, and
every run's claim, close and new ticket rode the code PR it belonged to. So every PR
carried `QUEUE.md` and ticket files, and every one of them went stale — and usually
conflicted — whenever `dev` moved; GitHub's server-side merge runs none of the merge
drivers that were written to cope (the code repo's `.gitattributes` has the
measurements: five laps on one PR in seventy minutes, re-ranks lost three times).

Here, **nothing is merged**. Every change is one small commit pushed straight to `main`
by `tools/ticket.mjs` (in the code repo), which pulls and retries when `main` has moved.
Two writers touching the *same* ticket or queue line is the only collision left, and
the tool refuses it instead of guessing — which is exactly what a claim lock wants.

- **A claim is real the moment it is pushed.** Every other run sees it on its next pull;
  two runs racing for one ticket cannot both push it. No marker branches, no waiting on
  a PR to merge.
- **`done` means "in review" until the PR merges.** `ticket.mjs done T-NNNN --pr N` sets
  `review`; this repo's **settle** workflow (every 15 minutes) marks the ticket `done` —
  and takes its queue line — when the code PR actually merges, or reopens it if the PR
  is closed unmerged. A ticket can no longer read done for work that did not land.
- **The owner's re-ranks can't be undone by a stale branch**, because there are no
  branches: Manager's reorder is a commit to `main`.

## Layout

| path | what it is |
|---|---|
| `QUEUE.md` | the priority order, top = next. **The owner orders it**; tools only append (beside the ticket a new one serves) and remove (on close) |
| `QUEUE_ORDER.md` | the re-rank ledger — one line per owner re-rank, with his instruction |
| `T-0000-0249/`, `T-0250-0499/`, … | **one file per ticket, in folders of 250 by number** (`T-1519` is in `T-1500-1749/`). New tickets land in the right folder automatically |
| `BOARD.md`, `tickets.json` | generated by `ticket.mjs board` — never committed on `main`; the **settle** workflow publishes them to the `board` branch, which is what Manager reads. It generates them with `--parked`, so both carry every open `hold`/`resume` pull request on the code repo with its reason and its age (T-1576) |

Working copy: the code repo clones this repo into `chicago/4d/tickets/` (gitignored
there) with `bash chicago/4d/tools/tickets.sh`; `check.sh` and `publish.sh` do it for
you. Hand edits in the clone (a finding added to a ticket body) are pushed with
`node tools/ticket.mjs sync -m "what changed"`.

## Decisions that need the owner — they stay in the queue

Owner, 2026-09-23: decisions were piling up where he had to go and prompt for each one;
*"leave them in the queue and add a comment with the question and options."* So:

```
node tools/ticket.mjs ask T-NNNN --question "…" \
     --option a="…" --option b="…" [--option c="…"] --rec a --why "…"
```

- the ticket **keeps its place in `QUEUE.md`** and gains `decision: pending`;
- its file gains a `## Decision needed` section — the question, lettered options, the
  recommendation and why;
- a comment line sits directly under its queue entry, so the question is readable in
  the queue itself:

  ```
  T-1157 — reconstruction GO for bands 3-5
  #   ? T-1157 DECISION: GO on bands 3-5? — (a) all now  (b) band 3 only  (c) hold — recommended: (a)
  ```

- `list --workable` and `claim` step over it until it is answered;
- Manager's 4D Board lists every pending one at the top under **Needs your decision**,
  with a button per option. Answering writes `decision: answered`,
  `decision_answer: <letter>` and a dated line into the ticket; the next run takes it
  from there (and `claim` removes the queue comment). By hand: make the same two
  front-matter edits.

Only four things are the owner's (AGENTS.md): rights and licensing, the depiction of
people, spending real money, and a decision that changes what the project IS. A missing
number is not one of them.

---

*Everything below is the queue's contract as it was written inside the code repo. It
still holds, with three differences stated above: claims are pushed commits, `done`
goes through `review`, and the folders of 250. Paths such as `tools/ticket.mjs` are
relative to `chicago/4d/` in the code repo.*

### The ticket queue (history)
...
5. `node tools/ticket.mjs done T-NNNN --pr N` once the PR is open. That sets `review`
   and pushes it here; the settle workflow makes it `done` (and takes its QUEUE line)
   when the PR merges. A ticket that turns out to need the owner gets
   `ask T-NNNN --question … --option a=… --option b=…` (it stays in the queue); one
   blocked on tooling gets `block --on "the missing thing"` — never silently abandoned.
