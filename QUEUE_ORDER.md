# RE-RANK LEDGER — owner decisions, first lines only; details remain in Git history.
#   2026-09-26  PRAIRIE AVENUE IS 1904, AND IT LEADS BAND 7, on the owner's instruction ("change those tickets and that year to 1904"; "keep south through time exactly in the same location"; "prioritize these 1904 prairie ave tickets over 1812"; "you can leave the 1888 scene if you need to"). Band 7 stays where it is. Inside it T-1250-T-1252 and T-0474-T-0477 now come before T-1286, T-1243 and T-0469-T-0472. T-1250 and T-0474-T-0477 are retitled to 1904, and all seven carry the ruling and the Glessner House sources (Sanborn 1911 vol. 3 key and sheets 20/28/35, Robinson 1886 plate 10), committed at chicago/reference/prairie-avenue/sanborn-1911-and-robinson-1886/.
#   2026-09-25  STRAIGHT TO BUILDING, on the owner's instruction ("we still have a lot of tickets to go before we build structures and that makes me sad"; "yes apply it, include T-1600"). T-1200 had slipped from ~14th to 28th workable as runs filed above it. Now: in-flight rows, then 5A's T-0419 (South Water's own corridor), 5B seating (T-1504, T-1579, T-1444, T-1600 moved here, T-1199, T-1482, T-0251), the frame budget (T-0135, T-1595), T-1200..T-1202. The seven BUILDING/INFRASTRUCTURE/STREET/landholding readings (T-1598, T-1599, T-1590-T-1592, T-1587, T-1588), T-1566 and T-1597 go below T-1202; the West ground fixes (T-1479, T-1570, T-1544, T-1460) go directly above T-1207; T-1601, T-1582, T-1602 go to band 9; T-1583 withdrawn (done by T-1584, #53). A standing rule heads band 0: new tickets below the next build unless dev is red.
#   2026-09-25  T-1504 TO THE HEAD OF 5B, on the owner's instruction: it is the one RESEARCH COMPLETION row that can add people (up to four) to the town, so it lands before T-1523/T-1199 seat anyone; the rest of band 10 writes ties or storage only and waits.
#   2026-09-25  T-0135 TO THE FOOT OF 5B, on the owner's instruction ("move"): the draw-call ceiling is read at the worst camera before the 5C districts add their roofs, since mobile is a release gate. Same day, T-1448 answered (a): mint the shop hands to the model's LOW band.
#   2026-09-25  TO THE BUILDINGS, on the owner's instruction: "get to the building tickets where we are actually filling in structures ... move up research that is important in its output and city impacting ... tickets that are small and trivial like resolving a single person keep at the bottom". T-1479 (blocks 28 and 45 re-cut, their structures re-seated) moved from RESEARCH COMPLETION to the head of 5A, ahead of seating; T-1555 (a writer rederive never runs) and T-1521 (the lap that can never finish a rebuild) to band 0, because the re-family and seating work is about to touch the resident layer they break; T-1547 (one inn's corner, 0.20 m) from 5A to the foot of 5D beside the signboards it concerns; the Toby pair T-1561/T-1562 out of band 1; band 10 re-ordered so the six single-person readings and the Toby pair sit last. T-1527 withdrawn: T-1526 (#14) cleared both reds on dev. No other ranked line moved.
#   2026-09-24  THE UNPLACED TICKETS PLACED INTO THE BANDS THAT EXIST, on the owner's instruction: "reorganize the queue and move any of the not placed tickets at the bottom into the correct chunk of work block area as it exists today". The 30 under the four MERGED-IN-NOT-YET-PLACED blocks and the four answered decisions of band 0 went to 0/1/2/4/5A/5B/5D/9/10 by subject, each appended at its band's foot (band 9's four PR-lap/stuck-report/budget tickets at its head); the empty WAITING band and the NOT-YET-PLACED headers were removed; no ranked ticket moved.
#   2026-09-17  ARRIVAL/JAUNTS (5F-5J) RE-ORDERED BY DEPENDENCY, on the owner's instruction to "review the
#               queue and the overall project and decompose these into tickets ... push these tickets in queue
#               in this or logical dependency order": inside each subsection the tickets with no predecessor
#               come first (T-1246, T-1292, T-1248, T-1277; T-1253 opens 5G), one City-summary ticket (T-1292)
#               was cut out of T-1276, and every other band kept its order.
#   2026-09-17  BAND 2 OPENED, on the owner's report that the loop was working T-0467 and T-1154 from the bottom: band 1 was all in flight or self-blocked and the band-2..5 gate hid the next 59 rows, so the topmost workable ticket really was in SOUTH THROUGH TIME. Only bands 3-5, which write reconstructed records, still wait on T-1157.
#   2026-09-17  THE RECONSTRUCTION BANDS, on the owner's written direction: confirm the research spend, then 1835 TOWN ANALYSIS, RECONSTRUCT RESIDENTS, BUSINESSES and STRUCTURES as ordered sections inserted between RESEARCH SPEND and SOUTH THROUGH TIME (T-1157 onward); sections over 15 tickets split into lettered sub-bands; four spend tickets enhanced.
#   2026-09-17  Owner: research spend, South Through Time, loop improvements, then research completion; remove the queue history and rank remaining epics.
#   2026-09-16  RESEARCH COMPLETION MOVED DIRECTLY ABOVE SOUTH THROUGH TIME, on the owner's
#   2026-09-15  RESEARCH COMPLETION GROUPED AT THE BOTTOM, on the owner's instruction after
#   2026-09-15  THE RESEARCH BAND IS CLOSED TO SUCCESSORS, on the owner's instruction: "Lots of
#   2026-09-14  WHAT THE GROUND LEFT BEHIND, on the owner's instruction after the day's merges:
#   2026-09-13  SOUTH THROUGH TIME BACK BELOW THE VISIBLE BANDS — the owner, hours after the
#   2026-09-10  "move the south through time section below section 5 and above section 6" — the
#   2026-09-10  THE WHOLE FILE, ON THE OWNER'S INSTRUCTION: "reorganize the queue to finish up
#   2026-09-07  THE SAND BAR, over three walkthrough screenshots. First instruction "place that
#   2026-09-07  "put several tickets at the top of the queue ... whatever is needed so they fire
#   2026-09-05  "Pull 802 up" — the owner, after T-0802's fault was caught live: T-0722 sat
#   2026-09-05  ...AND THE REST OF THE FILE, same instruction: "And other logical ordering
#   2026-09-05  THE CORE DATASET, RE-ISSUED AND REBUILT. The owner repeated his 2026-09-04
#   2026-09-05  "Rank T-0727" — under the drain band, per the site-budget ruling.
#   2026-09-05  drain the open-PR queue (band below); pace sliders + framed arrival (#907).
#   2026-09-04  RESTORED after merge-queue.mjs clobbered it: "put it back with all of the
#   2026-09-04  the core dataset before more reading · the grades questioned (T-0699/0692/0693)
#   2026-09-03  best-yield research first · consolidate every few, not at the end · the Sauganash
#   2026-08-30  "lots of nothing happened in the city which is bad" — 41 merges, 0 buildings
#   2026-08-29 (x2), 2026-08-28, 2026-08-27, 2026-08-23
#   2026-09-13  SOUTH THROUGH TIME TO THE TOP, and every unplaced ticket ranked. The owner:
