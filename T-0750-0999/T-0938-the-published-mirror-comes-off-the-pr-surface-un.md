---
id: T-0938
title: The published mirror comes off the PR surface: untrack site/4d/ and have deploy.yml publish it
state: done
epic: META
requested_by: owner
seen: false
...
The published mirror comes off the PR surface: untrack site/4d/ and have deploy.yml publish it.
...
`/v/release-vNNN/` snapshots and `chicago-4d-promote-to-prod.yml` read the COMMITTED mirror."
  ...
  * **There are no `/v/` snapshots.** `site/4d/` holds `build.json`, `data`,
...
So the shape is the one the parent designed. What DOES have to be carried:
  ...
  * `deploy.yml` uploads `path: site` from the runner, so it must run `tools/publish.sh` on the
    main checkout before the upload — and the dev-preview step copies `_dev-src/site/4d`
  ...
  * ~40 tools resolve `../../../site/4d` for their `--published` mode, and
...
**Acceptance:** `site/4d/**` untracked and ignored; `deploy.yml` publishes before it
